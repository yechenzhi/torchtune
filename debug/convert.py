...
from transformers import LlamaConfig, LlamaForCausalLM, LlamaTokenizer, PreTrainedTokenizerFast, LlamaForSequenceClassification
...
def write_model(
    ...
    if not os.path.isfile(os.path.join(input_base_path, "original_params.json")):
    ...
    params = read_json(os.path.join(input_base_path, "original_params.json"))
    ...
    if num_shards == 1:
        ...
        loaded = torch.load(os.path.join(input_base_path, "original/consolidated.00.pth"), map_location="cpu")
        # loaded = torch.load(os.path.join(input_base_path, "meta_model_0.pt"), map_location="cpu")
    else:
        # Sharded
        loaded = [
            # torch.load(os.path.join(input_base_path, f"original/consolidated.{i:02d}.pth"), map_location="cpu")
            # torch.load(os.path.join(input_base_path, f"meta_model_{i:02d}.pt"), map_location="cpu")
    ...
    model = LlamaForSequenceClassification.from_pretrained(tmp_model_path, num_labels=10, torch_dtype=torch.float16, low_cpu_mem_usage=True)
